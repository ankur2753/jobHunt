...
class InstahyrePlaywright:
    def __init__(self, cookies_file: str = "../../personal_details/instahyre_cookies.json"):
        script_dir = Path(__file__).parent
        self.cookies_file = (script_dir / cookies_file).resolve()
        self.base_url = "https://www.instahyre.com"
    ...
    async def setup_driver(self, headless: bool = False, port: int = 3000):
        ...
        if os.path.exists(self.cookies_file):
            print("✅ Loading existing session state for Instahyre...")
            self.context = await self.browser.new_context(
                storage_state=self.cookies_file,
                user_agent="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"
            )
        else:
            print("⚠️ No session file found. Starting fresh context for Instahyre.")
            self.context = await self.browser.new_context()
    ...
    async def is_logged_in(self):
        ...
        try:
            ...
            # A common selector for logged-in users on Instahyre is the user dropdown or logout link
            await self.page.wait_for_selector("#user-dropdown, a[href='/logout/']", timeout=5000)
            return True
        except:
            return False
    ...
    async def login_manually_and_save(self):
        ...
        try:
            await self.page.wait_for_selector("#user-dropdown, a[href='/logout/']", timeout=120000) 
            await self.save_session()
            print("✅ Login successful and cookies captured for Instahyre!")
